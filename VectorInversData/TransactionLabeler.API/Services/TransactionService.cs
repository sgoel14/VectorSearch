            ...
            try
            {
                ...
                var chatOptions = new ChatOptions
                {
                    ...
                    [
                        ...
                        AIFunctionFactory.Create(
                            ...
                            "Finds top transactions for ANY specific category using vector search. Use for queries like 'Top 10 transactions for marketing costs', 'Show me travel expenses', 'Show me car repair transactions', 'Find transactions for staff drink and food', 'Show me marketing transactions', 'List transactions for travel', 'Get transactions for office supplies', 'Search 10 transactions for car repair', 'Find 5 transactions for marketing', 'Get transactions for category [any category]', 'can you find transactions for car repair costs', 'top 5 transactions for car repair costs'. ANY query containing 'transactions', 'find transactions', 'get transactions', 'show transactions', 'list transactions', 'top X transactions' should use this tool. Extract the category type from the user's query. If no number specified, use 10. Only include year parameter if explicitly mentioned in the query."
                        ),
                        AIFunctionFactory.Create(
                ...
                var chatHistory = new List<ChatMessage>
                {
                    new(ChatRole.System, @"
                        ...
                        - IMPORTANT: Always calculate relative dates from TODAY's date. If today is 2025-08-01, then 'last 6 months' means from 2025-02-01 to 2025-08-01
                        - For relative dates like 'last 6 months', calculate from TODAY backwards: if today is 2025-08-01, then 'last 6 months' = startDate = '2025-02-01' (6 months ago), endDate = '2025-08-01' (today)
                        - For year-only references like '2022', set year = 2022, not as startDate/endDate
                        - Only use valid years (1900-2100) - ignore invalid years like 0
                        ...
                        - TOOL SELECTION: Use SearchCategories ONLY when the user wants to explore available categories (e.g., 'what categories are available', 'show me categories', 'list all categories'). Use GetTopTransactionsForCategory when the user asks for transaction data, transaction details, transaction lists, or ANY query containing words like 'transactions', 'find transactions', 'get transactions', 'show transactions', 'list transactions', 'top X transactions', 'top transactions'. PRIORITY: If a query contains the word 'transactions', it is ALWAYS a transaction request, regardless of other context.
                        ...
                        - TRANSACTION QUERIES RULE: For ANY query containing 'transactions', 'find transactions', 'get transactions', 'show transactions', 'list transactions', 'top X transactions', 'top transactions', or asking for transaction details, use GetTopTransactionsForCategory. Do NOT use SearchCategories for transaction requests. The word 'transactions' in any form indicates a transaction request. CRITICAL: If the user asks for 'top X transactions' or any variation with 'transactions', ALWAYS use GetTopTransactionsForCategory, never SearchCategories. CRITICAL: The presence of the word 'transactions' in ANY query means it's a transaction request, not a category exploration request.
                        - DATE PARAMETER RULE: Always provide valid date parameters. For relative dates like 'last 6 months', calculate from TODAY backwards. Examples: 'last 6 months' (if today is 2025-08-01) = startDate = '2025-02-01', endDate = '2025-08-01'. 'last 3 months' = startDate = '2025-05-01', endDate = '2025-08-01'. For year references like '2022', use year parameter, not startDate/endDate. Never use invalid years (0, negative, or outside 1900-2100 range). If no specific year is mentioned, do NOT include year parameter or set it to null.
                        "),
